...
def check_address(address: str):
    """
    seid keys parse <address>
    """
    check = subprocess.run(["seid", "keys", "parse",
                            f"{address}",
                            '--output=json'],
...
def get_balance(address: str, node: str, chain_id: str):
    """
    seid query bank balances <address> <node> <chain-id>
    """
    balance = subprocess.run(["seid", "query", "bank", "balances",
                              f"{address}",
                              f"--node={node}",
                              f"--chain-id={chain_id}",
                              '--output=json'],
...
def get_tx_info(hash_id: str, node: str, chain_id: str):
    """
    seid query tx <tx-hash> <node> <chain-id>
    """
    tx_gaia = subprocess.run(['seid', 'query', 'tx',
                              f'{hash_id}',
                              f'--node={node}',
                              f'--chain-id={chain_id}',
                              '--output=json'],
...
def tx_send(request: dict):
    ...
    - "node"
    - "chain_id"
    seid tx bank send <from address> <to address> <amount>
                       <fees> <node> <chain-id>
                       --keyring-backend=test -y

    """
    tx_gaia = subprocess.run(['seid', 'tx', 'bank', 'send',
                              f'{request["sender"]}',
                              f'{request["recipient"]}',
                              f'{request["amount"]}',
                              f'--fees={request["fees"]}',
